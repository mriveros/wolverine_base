...
=begin
    if params[:form_buscar_personas_nombre_persona].present?

      cond << "personas.nombre_persona ilike ?"
      args << "%#{params[:form_buscar_personas_nombre_persona]}%"

    end

    if params[:form_buscar_personas_apellido_persona].present?

      cond << "personas.apellido_persona ilike ?"
      args << "%#{params[:form_buscar_personas_apellido_persona]}%"
...
=begin
    ...
    if params[:form_buscar_personas] && params[:form_buscar_personas][:pais_id].present?

      cond << "departamentos.pais_id = ?"
      args << params[:form_buscar_personas][:pais_id]

    end
    if params[:form_buscar_personas] && params[:form_buscar_personas][:departamento_id].present?

      cond << "jurisdicciones.departamento_id = ?"
      args << params[:form_buscar_personas][:departamento_id]

    end

    if params[:form_buscar_personas] && params[:form_buscar_personas][:jurisdiccion_id].present?

      cond << "personas.jurisdiccion_id = ?"
      args << params[:form_buscar_personas][:jurisdiccion_id].present?
...
=end
  ...
  def guardar
    ...
    #persona = Persona.where("documento_persona = ? and tipo_documento_id = ? and nacionalidad_id = ?", @persona.documento_persona, @persona.tipo_documento_id, @persona.nacionalidad_id)

    #if persona.present?

#      valido = false
#      @msg += "Este persona ya ha sido registrada.<br />"
 ...
 #   if valido
      ...
      if @persona.save
        ...
        @persona_ok = true
        #pust 'ERROR: No se ha podido registrar el persona.'

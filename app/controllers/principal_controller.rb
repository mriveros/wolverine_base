...
class PrincipalController < ApplicationController
  ...
  def index
	...
	def index_v2
		...
		#@rol_usuario = Perfil.where("usuario_id = ? ", current_usuario.id )
		...
		#@informaciones = Informacion.where("id in (?) or id in (?)", informaciones_ids, informaciones_sin_roles_ids).order('fecha_publicacion DESC').limit(15)
		#@informaciones = Informacion.where("id in (?) or id in (?)", informaciones_ids, informaciones_sin_roles_ids).order('fecha_publicacion DESC')
		...
		#@notas_niveles = VNotaNivel.order('nota_nivel_id DESC').limit(5)
		#@notas_niveles = VNotaNivel.where("periodo_escolar = ?",periodo).order('fecha_desde ASC')
		#@calendario_examenes = VNotaNivel.order('fecha_desde ASC')
		#@calendario_examenes_escolar_inicial = VNotaNivel.where("nivel_educativo_id = ?", 1).order('fecha_desde ASC')
		#@calendario_examenes_escolar_basica = VNotaNivel.where("nivel_educativo_id = ?", 2).order('fecha_desde ASC')
		#@calendario_examenes_educacion_media = VNotaNivel.where("nivel_educativo_id = ?", 3).order('fecha_desde ASC')
		#@calendario_examenes_educacion_superior = VNotaNivel.where("nivel_educativo_id = ?", 4).order('fecha_desde ASC')
		
		#@calendario_examenes_escolar_inicial = VNotaNivel.where("nivel_educativo_id = ?", PARAMETRO[:nivel_educativo_inicial]).order('fecha_desde ASC')
		#@calendario_examenes_escolar_basica = VNotaNivel.where("nivel_educativo_id = ?", PARAMETRO[:nivel_educativo_basico]).order('fecha_desde ASC')
		#@calendario_examenes_educacion_media = VNotaNivel.where("nivel_educativo_id = ?", PARAMETRO[:nivel_educativo_medio]).order('fecha_desde ASC')
		#@calendario_examenes_educacion_superior = VNotaNivel.where("nivel_educativo_id = ?", PARAMETRO[:nivel_educativo_superior]).order('fecha_desde ASC')
...
=begin
		...
		@rol_usuario.each do |r|

            if PARAMETRO[:rol_nivel_inicial_administrador] == r.rol_id || PARAMETRO[:rol_nivel_eeb_administrador] == r.rol_id || PARAMETRO[:rol_inclusiva_administrador] == r.rol_id || PARAMETRO[:rol_educacion_indigena_administrador] == r.rol_id|| PARAMETRO[:rol_permanente_administrador] == r.rol_id

              puts "ACCESO EDUCACION INICIAL - BASICA"
              @calendario_examenes_escolar_inicial = VNotaNivel.where("nivel_educativo_id = ?", PARAMETRO[:nivel_educativo_inicial]).order('fecha_hasta ASC')
							@calendario_examenes_escolar_basica = VNotaNivel.where("nivel_educativo_id = ?", PARAMETRO[:nivel_educativo_basico]).order('fecha_hasta ASC')
							
						else
								if PARAMETRO[:rol_nivel_medio_cientifico_administrador]== r.rol_id || PARAMETRO[:rol_tecnico_pedagogico_media]== r.rol_id || PARAMETRO[:rol_nivel_medio_tecnico_administrador]== r.rol_id

									puts "ACCESO EDUCACION MEDIA"
             
									@calendario_examenes_educacion_media = VNotaNivel.where("nivel_educativo_id = ?", PARAMETRO[:nivel_educativo_medio]).order('fecha_hasta ASC')
								else
									if PARAMETRO[:rol_nivel_superior_administrador]== r.rol_id

										puts "ACCESO EDUCACION SUPERIOR"
    ...
    if valido
      ...
      if @usuario.save
        ...
        if perfil.save
          ...
          NotificarTutor.email(@usuario, password).deliver
          @usuario_ok = true
    ...
    if persona.present?
      ...
      if valido
        ...
        if @usuario.save
          ...
          NotificarTutor.email_recuperar_password(@usuario, password).deliver
          @usuario_ok = true
